...
class SimulationLogManager:
    ...
    def get_bags_fm_logger(self) -> PlatformActionLogger:
        """Get the Bags.fm platform action logger"""
        if not hasattr(self, 'bags_fm_logger') or self.bags_fm_logger is None:
            self.bags_fm_logger = PlatformActionLogger("bags_fm", self.simulation_dir)
        return self.bags_fm_logger
