...
from monitor_risk_bridge import (
    MonitorRiskBridge,
    inject_risk_context,
    inject_token_context,
)
...
BAGS_FM_ACTION_TYPE_MAP = {
    'browse_tokens': 'BROWSE_TOKENS',
    'buy_token': 'BUY_TOKEN',
    'sell_token': 'SELL_TOKEN',
    'view_portfolio': 'VIEW_PORTFOLIO',
    'launch_token': 'LAUNCH_TOKEN',
    'comment_on_token': 'COMMENT_ON_TOKEN',
    'do_nothing': 'DO_NOTHING',
    'sign_up': 'SIGN_UP',
}
...
def _load_bags_fm_profiles(profile_path: str) -> List[Dict[str, Any]]:
    """Load Bags.fm agent profiles from JSON."""
    with open(profile_path, 'r', encoding='utf-8') as f:
        return json.load(f)


def _build_bags_fm_agent_graph(
    profiles: List[Dict[str, Any]],
    model,
) -> AgentGraph:
    """Build an AgentGraph for Bags.fm from profile dicts.

    Each profile has: user_id, name, description, risk_tolerance, user_profile.
    Agents are created with simulation=bags_fm_simulation so they use
    BagsFmAction / BagsFmEnvironment / BagsFmPromptBuilder.
    """
    agent_graph = AgentGraph()
...
def _fetch_bags_fm_actions_from_db(
    db_path: str,
    last_rowid: int,
    agent_names: Dict[int, str],
) -> Tuple[List[Dict[str, Any]], int]:
    """Fetch new Bags.fm actions from the trace table."""
    actions = []
    new_last_rowid = last_rowid
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            action_type = BAGS_FM_ACTION_TYPE_MAP.get(action, action.upper())
        ...
        print(f"Failed to read Bags.fm actions from database: {e}")
...
async def run_bags_fm_simulation(
    config: Dict[str, Any],
    simulation_dir: str,
    action_logger: Optional[PlatformActionLogger] = None,
    main_logger: Optional[SimulationLogManager] = None,
    ...
    """Run Bags.fm token launch simulation standalone."""
    result = PlatformSimulation()

    def log_info(msg):
        if main_logger:
            main_logger.info(f"[Bags.fm] {msg}")
        print(f"[Bags.fm] {msg}")

    log_info("Initializing...")

    _standalone_monitor_bridge = MonitorRiskBridge()
    log_info(f"Monitor Risk Bridge: {_standalone_monitor_bridge.api_url}")

    model = create_model(config, use_boost=False)

    profile_path = os.path.join(simulation_dir, "bags_fm_profiles.json")
    if not os.path.exists(profile_path):
        log_info(f"Error: Profile file not found: {profile_path}")
        return result

    profiles = _load_bags_fm_profiles(profile_path)
    result.agent_graph = _build_bags_fm_agent_graph(profiles, model)
    ...
    db_path = os.path.join(simulation_dir, "bags_fm_simulation.db")
    if not is_resume and os.path.exists(db_path):
        os.remove(db_path)
    ...
    if not is_resume:
        event_config = config.get("event_config", {})
        initial_tokens = event_config.get("initial_tokens", [])
        ...
        if initial_tokens:
            ...
            await result.env.step({agent_0: seed_actions})
            log_info(f"Seeded {len(seed_actions)} initial tokens")
            ...
            if action_logger:
                for token in initial_tokens:
                    action_logger.log_action(
                        ...
                        agent_name=agent_names.get(0, "Agent_0"),
                        action_type="LAUNCH_TOKEN",
                        action_args={"ticker": token.get("ticker", "")},
                    )
    ...
    belief_tracker = BeliefTracker(config, simulation_dir, "bags_fm")
    log_info(f"Belief tracking: {len(belief_tracker.topics)} topics")
    ...
    for round_num in range(start_round, total_rounds):
        ...
        if cross_platform_log:
            for agent_id, agent in active_agents:
                digest = cross_platform_log.build_digest(
                    agent_id, exclude_platform="bags_fm"
                )
                if digest:
                    inject_cross_platform_context(agent, digest)

        risk_prompt = _standalone_monitor_bridge.get_risk_prompt()
        token_prompt = _standalone_monitor_bridge.get_token_prompt()
        for _, agent in active_agents:
        ...
        actual_actions, last_rowid = _fetch_bags_fm_actions_from_db(
            db_path, last_rowid, agent_names
        )
        ...
        if cross_platform_log and actual_actions:
            cross_platform_log.record("bags_fm", actual_actions)
...
async def run_synchronized_simulation(
    ...
    bags_fm_logger: Optional[PlatformActionLogger] = None,
    main_logger: Optional[SimulationLogManager] = None,
    max_rounds: Optional[int] = None,
    start_round: int = 0,
    cross_platform_log: Optional[CrossPlatformLog] = None,
    ...
    has_bags_fm: bool = False,
) -> Tuple[Optional[PlatformSimulation], Optional[PlatformSimulation], Optional[PlatformSimulation], Optional[PlatformSimulation]]:
    """Run all platforms in lock-step: one round at a time across all platforms.

    Returns (twitter_result, reddit_result, polymarket_result, bags_fm_result).
    """
    def log_info(msg):
        if main_logger:
    ...
    bags_fm_db = os.path.join(simulation_dir, "bags_fm_simulation.db")
    ...
    if has_bags_fm:
        bags_fm_result = PlatformSimulation()
        profile_path = os.path.join(simulation_dir, "bags_fm_profiles.json")
        profiles = _load_bags_fm_profiles(profile_path)
        bags_fm_result.agent_graph = _build_bags_fm_agent_graph(profiles, model)
        for agent_id, agent in bags_fm_result.agent_graph.get_agents():
            if agent_id not in agent_names:
                agent_names[agent_id] = getattr(agent, 'name', f'Agent_{agent_id}')
        if start_round == 0 and os.path.exists(bags_fm_db):
            os.remove(bags_fm_db)
        bags_fm_result.env = wonderwall.make(
            agent_graph=bags_fm_result.agent_graph,
            simulation=bags_fm_simulation,
            database_path=bags_fm_db,
            semaphore=60,
        )
        await bags_fm_result.env.reset()
        log_info("[Bags.fm] Environment ready")
    ...
    bags_fm_belief = BeliefTracker(config, simulation_dir, "bags_fm") if has_bags_fm else None
    ...
    if bags_fm_logger:
        bags_fm_logger.log_simulation_start(config)
    ...
    # ── Seed Bags.fm initial tokens (round 0) ──
    initial_tokens = event_config.get("initial_tokens", [])
    if start_round == 0 and initial_tokens and bags_fm_result:
        agent_0 = bags_fm_result.env.agent_graph.get_agent(0)
        seed_actions = []
        for token in initial_tokens:
            seed_actions.append(ManualAction(
                action_type="launch_token",
        ...
        if seed_actions:
            await bags_fm_result.env.step({agent_0: seed_actions})
            for t in initial_tokens:
                log_info(f"  Token: ${t['ticker']} ({t['name'][:40]})")
            log_info(f"[Bags.fm] Seeded {len(seed_actions)} tokens")
    ...
    for round_num in range(start_round, total_rounds):
        ...
        if polymarket_result:
            ...
            if active:
                ...
                risk_prompt = monitor_bridge.get_risk_prompt()
                for _, agent in active:
                    inject_risk_context(agent, risk_prompt)
        ...
        if bags_fm_result:
            active = get_active_agents_for_round(bags_fm_result.env, config, simulated_hour, round_num)
            if active:
                if memory_ctx:
                    for _, agent in active:
                        inject_round_memory(agent, memory_ctx)
                ...
                if cross_platform_log:
                    for agent_id, agent in active:
                        digest = cross_platform_log.build_digest(agent_id, exclude_platform="bags_fm")
                        if digest:
                            inject_cross_platform_context(agent, digest)
                ...
                risk_prompt = monitor_bridge.get_risk_prompt()
                token_prompt = monitor_bridge.get_token_prompt()
                for _, agent in active:
                    inject_risk_context(agent, risk_prompt)
                    inject_token_context(agent, token_prompt)

                async def _step_bags_fm(active_agents=active):
                    actions = {agent: LLMAction() for _, agent in active_agents}
                    await bags_fm_result.env.step(actions)
                    return active_agents

                platform_tasks.append(("bags_fm", _step_bags_fm()))
        ...
        if "bags_fm" in platform_results:
            actual_actions, bags_fm_last_rowid = _fetch_bags_fm_actions_from_db(bags_fm_db, bags_fm_last_rowid, agent_names)
            if bags_fm_belief:
                bags_fm_belief.after_round(bags_fm_db, bags_fm_result.env, platform_results["bags_fm"], round_num, actual_actions)
            if cross_platform_log and actual_actions:
                cross_platform_log.record("bags_fm", actual_actions)
            round_memory.record("bags_fm", round_num, actual_actions)
            if bags_fm_logger:
                bags_fm_logger.log_round_start(round_num + 1, simulated_hour)
                for a in actual_actions:
                    bags_fm_logger.log_action(round_num=round_num+1, agent_id=a['agent_id'], agent_name=a['agent_name'], action_type=a['action_type'], action_args=a['action_args'])
                bags_fm_logger.log_round_end(round_num + 1, len(actual_actions))
            bags_fm_result.total_actions += len(actual_actions)
        ...
        if (round_num + 1) % 5 == 0 or round_num == 0:
            ...
            if bags_fm_result:
                parts.append(f"BF:{bags_fm_result.total_actions}")
            log_info(
    ...
    # Save trajectories
    for tracker, name in [(twitter_belief, "Twitter"), (reddit_belief, "Reddit"), (polymarket_belief, "Polymarket"), (bags_fm_belief, "Bags.fm")]:
        if tracker:
    ...
    # End loggers
    for logger, name in [(twitter_logger, "Twitter"), (reddit_logger, "Reddit"), (polymarket_logger, "Polymarket"), (bags_fm_logger, "Bags.fm")]:
        if logger:
            logger.log_simulation_end(total_rounds, 0)

    return twitter_result, reddit_result, polymarket_result, bags_fm_result
...
async def main():
    ...
    parser.add_argument(
        '--bags-fm-only',
        action='store_true',
        help='Run Bags.fm token launch simulation only'
    )
    parser.add_argument(
        '--max-rounds',
        type=int,
    ...
    bags_fm_logger = log_manager.get_bags_fm_logger()
    ...
    log_manager.info(f"  - Bags.fm actions: bags_fm/actions.jsonl")
    log_manager.info("=" * 60)
    ...
    bags_fm_result: Optional[PlatformSimulation] = None
    ...
    if args.env_only:
        ...
        has_bags_fm = os.path.exists(os.path.join(simulation_dir, "bags_fm_profiles.json"))

        # Create market-media bridge when both social media AND polymarket are running
        has_social = has_twitter or has_reddit
        bridge = MarketMediaBridge() if (has_social and (has_polymarket or has_bags_fm)) else None
        if bridge:
        ...
        elif args.bags_fm_only:
            bags_fm_result = await run_bags_fm_simulation(config, simulation_dir, bags_fm_logger, log_manager, args.max_rounds, args.start_round, cross_platform_log=xp_log)
        else:
            # Multiple platforms: use synchronized mode so they step together
            # This ensures the Market-Media Bridge data is never stale
            platform_count = sum([has_twitter, has_reddit, has_polymarket, has_bags_fm])
            platform_names = [p for p, h in [("Twitter", has_twitter), ("Reddit", has_reddit), ("Polymarket", has_polymarket), ("Bags.fm", has_bags_fm)] if h]
            log_manager.info(f"Launching {platform_count} platforms in SYNCHRONIZED mode: {', '.join(platform_names)}")

            twitter_result, reddit_result, polymarket_result, bags_fm_result = await run_synchronized_simulation(
                config=config,
                simulation_dir=simulation_dir,
                twitter_logger=twitter_logger,
                reddit_logger=reddit_logger,
                polymarket_logger=polymarket_logger,
                bags_fm_logger=bags_fm_logger,
    ...
    if bags_fm_result and bags_fm_result.env:
        await bags_fm_result.env.close()
        log_manager.info("[Bags.fm] Environment closed")
