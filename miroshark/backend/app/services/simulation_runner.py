...
from .agent_credentials import SimulationKeyring, content_hash as ac_content_hash
...
class AgentAction:
    ...
    # KYA attestation — populated when the action is parsed in _read_action_log.
...
class SimulationRunState:
    ...
    def add_action(self, action: AgentAction):
        ...
        elif action.platform == "bags_fm":
            self.bags_fm_actions_count += 1
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "bags_fm_current_round": self.bags_fm_current_round,
            "twitter_simulated_hours": self.twitter_simulated_hours,
            "reddit_simulated_hours": self.reddit_simulated_hours,
            "polymarket_simulated_hours": self.polymarket_simulated_hours,
            "bags_fm_simulated_hours": self.bags_fm_simulated_hours,
            "twitter_running": self.twitter_running,
            "reddit_running": self.reddit_running,
            "polymarket_running": self.polymarket_running,
            "bags_fm_running": self.bags_fm_running,
            "twitter_completed": self.twitter_completed,
            "reddit_completed": self.reddit_completed,
            "polymarket_completed": self.polymarket_completed,
            "bags_fm_completed": self.bags_fm_completed,
            "twitter_actions_count": self.twitter_actions_count,
            "reddit_actions_count": self.reddit_actions_count,
            "polymarket_actions_count": self.polymarket_actions_count,
            "bags_fm_actions_count": self.bags_fm_actions_count,
            "total_actions_count": self.twitter_actions_count + self.reddit_actions_count + self.polymarket_actions_count + self.bags_fm_actions_count,
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        elif platform == "bags_fm":
            script_name = "run_parallel_simulation.py"
            state.bags_fm_running = True
        else:
            script_name = "run_parallel_simulation.py"
            state.twitter_running = True
            state.reddit_running = True
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        bags_fm_actions_log = os.path.join(sim_dir, "bags_fm", "actions.jsonl")
        ...
        if os.path.exists(bags_fm_actions_log):
            bags_fm_position = os.path.getsize(bags_fm_actions_log)
        ...
        try:
            while process.poll() is None:  # Process still running
                ...
                # Read Bags.fm action logs
                if os.path.exists(bags_fm_actions_log):
                    bags_fm_position = cls._read_action_log(
                        bags_fm_actions_log, bags_fm_position, state, "bags_fm"
                    )
            ...
            if os.path.exists(bags_fm_actions_log):
                cls._read_action_log(bags_fm_actions_log, bags_fm_position, state, "bags_fm")
    ...
    def _read_action_log(
        ...
        # Signed-action sink (sibling of the subprocess-owned actions.jsonl).
        keyring = SimulationKeyring.lookup(state.simulation_id)
        signed_log_path = os.path.join(os.path.dirname(log_path), "signed_actions.jsonl")
                ...
                for line in f:
                    ...
                    if line:
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    if platform == "twitter":
                                        ...
                                        state.bags_fm_completed = True
                                        state.bags_fm_running = False
                                        logger.info(f"Bags.fm simulation completed: {state.simulation_id}, total_rounds={action_data.get('total_rounds')}, total_actions={action_data.get('total_actions')}")
                                ...
                                # Update round info (from round_end event)
                                    ...
                                    if platform == "twitter":
                                        ...
                                        if round_num > state.bags_fm_current_round:
                                            state.bags_fm_current_round = round_num
                                        state.bags_fm_simulated_hours = simulated_hours

                                    # Overall round is the maximum of all platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Overall time is the maximum of all platforms
                                    state.simulated_hours = max(state.twitter_simulated_hours, state.reddit_simulated_hours, state.polymarket_simulated_hours, state.bags_fm_simulated_hours)
                                ...
                                # the display by a full round-duration).
                                    ...
                                    if platform == "twitter" and round_num > state.twitter_current_round:
                                        ...
                                        state.polymarket_current_round = round_num
                                    elif platform == "bags_fm" and round_num > state.bags_fm_current_round:
                                        state.bags_fm_current_round = round_num
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                            ...
                            # KYA: sign the action with the agent's ed25519 key.
                            # Private keys live only inside the backend process; the
                            # OASIS subprocess never sees them.
                            if keyring is not None:
                                cred = keyring.get(action.agent_id)
                                if cred:
                                    canonical_payload = {
                                        ...
                                        "content_hash": ac_content_hash({
                                            "action_args": action.action_args,
                                            "result": action.result,
                                        }),
                                        "timestamp": action.timestamp,
                                    }
                                    attest = keyring.sign_for_agent(action.agent_id, canonical_payload)
                                    if attest:
                                        action.agent_did = attest["did"]
                                        action.content_hash = canonical_payload["content_hash"]
                                        action.signature = attest["signature"]
                                        try:
                                            with open(signed_log_path, "a", encoding="utf-8") as sf:
                                                sf.write(json.dumps({
                                                    **action.to_dict(),
                                                    "signed_payload": canonical_payload,
                                                }) + "\n")
                                        except Exception as e:
                                            logger.debug("failed to append signed action: %s", e)
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        bags_fm_log = os.path.join(sim_dir, "bags_fm", "actions.jsonl")
        ...
        bags_fm_enabled = os.path.exists(bags_fm_log)
        ...
        if polymarket_enabled and not state.polymarket_completed:
            return False
        if bags_fm_enabled and not state.bags_fm_completed:
            return False

        # At least one platform is enabled and completed
        return twitter_enabled or reddit_enabled or polymarket_enabled or bags_fm_enabled
