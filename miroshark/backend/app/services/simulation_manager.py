...
from .agent_credentials import SimulationKeyring
...
class SimulationState:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "enable_bags_fm": self.enable_bags_fm,
...
class SimulationManager:
    ...
    def _load_simulation_state(self, simulation_id: str) -> Optional[SimulationState]:
        ...
        state = SimulationState(
            ...
            enable_bags_fm=data.get("enable_bags_fm", False),
    ...
    def create_simulation(
        ...
        Args:
            ...
            enable_bags_fm: Whether to enable Bags.fm simulation
    ...
    def prepare_simulation(
        ...
        try:
            ...
            # Mint ed25519 keypairs for every agent + a per-simulation root key.
            # Keys live inside sim_dir/agent_keys.json — never written to Neo4j.
            try:
                keyring = SimulationKeyring.for_simulation(state.simulation_id, sim_dir)
                keyring.root_credential()  # mint the sim-root key eagerly
                agent_ids = [p.user_id for p in profiles if p.user_id is not None]
                creds = keyring.mint_batch(agent_ids)
                for p in profiles:
                    if p.user_id is None:
                        continue
                    cred = creds.get(str(p.user_id))
                    if cred:
                        p.did = cred.did
                logger.info(
                    "KYA: minted %d agent keypairs + sim-root key for simulation %s",
                    len(creds), state.simulation_id,
                )
            except Exception as e:
                logger.error("KYA key minting failed for %s: %s", state.simulation_id, e)
            ...
            if state.enable_bags_fm:
                generator.save_profiles(
                    profiles=profiles,
                    file_path=os.path.join(sim_dir, "bags_fm_profiles.json"),
                    platform="polymarket"  # Bags.fm uses polymarket format (risk_tolerance)
                )
