...
def create_app(config_class=Config):
    ...
    try:
        ...
        app.config['NEO4J_STORAGE'] = neo4j_storage
        if should_log_startup:
    ...
    # --- Posture A: idempotent static demo-graph seed (ZERA-600) -----------
    # Loads a small, version-controlled, fully synthetic persona graph ONLY
    # when Neo4j is empty, so the documented one-command path can run a
    # MiroShark simulation with no manual ingest step. No-op on a populated
    # volume; never raises into startup (degrades to Posture B at sim time).
    # Skip on the debug reloader's parent process to avoid a double load.
    if not (debug_mode and not is_reloader_process):
        try:
            from .seed import ensure_seed_graph
            ensure_seed_graph(app.extensions.get('neo4j_storage'))
        except Exception as e:
            logger.error("Static seed step failed (non-fatal): %s", e)
    ...
    # Register blueprints
    from .api import graph_bp, simulation_bp, report_bp, templates_bp, settings_bp, observability_bp, mcp_bp, docs_bp, feed_bp, share_bp, watch_bp, monitor_bp
    from .api.token_bridge import token_bridge_bp
    from .api.agents import agents_bp, verify_bp
    ...
    app.register_blueprint(token_bridge_bp, url_prefix='/api/simulation')
