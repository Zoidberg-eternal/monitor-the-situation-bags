    ...
    Request (JSON):
        {
            ...
            "enable_bags_fm": false          // Optional, default false
        }
...
def start_simulation():
    ...
    try:
        ...
        if platform not in ['twitter', 'reddit', 'polymarket', 'bags_fm', 'parallel']:
            return jsonify({
                "success": False,
                "error": _t(
                    f"Invalid platform type: {platform}, options: twitter/reddit/polymarket/bags_fm/parallel",
                    f"无效的平台类型:{platform},可选项:twitter/reddit/polymarket/bags_fm/parallel",
                    locale,
                )
            }), 400
